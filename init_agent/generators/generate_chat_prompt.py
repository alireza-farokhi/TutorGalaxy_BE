...
import random
import openai
from ..chat_prompt_creator.categories_chat_prompt import C_prompt

def get_chat_prompt_method(self,chat_prompt):
    max_retries = 2
    retries = 0
    cat = self.topic_details.get('category')
    prompt = C_prompt.get(cat).format(self.conversation_so_far)
